...
import { initialCustomerState } from "@/reducerStates/customerState";
import {
  bulkDeleteCustomers,
  deleteCustomerById,
  getCustomers,
...
import CustomerGroupFilter from "@/components/common/filter/CustomerGroupFilter";
...
export default function CustomerTable() {
   ...
   */
  const fetchCustomersDebounced = useCallback(
    debounce(async (page: number, limit: number) => {
      ...
      try {
        ...
        dispatch({ type: "SET_CUSTOMERS", payload: data.data.items });
    ...
    [page, limit, sorting, search, status, createdBy, customerGroupId],
  );
   ...
   */
  useEffect(() => {
    fetchCustomersDebounced(page, limit);

    return () => {
      fetchCustomersDebounced.cancel();
    };
  }, [page, limit, fetchCustomersDebounced]);
  ...
  const deleteCustomer = useCallback(async () => {
    ...
    try {
      const data = await deleteCustomerById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchCustomersDebounced, customers.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteCustomers(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchCustomersDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Customer>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === customers.length &&
            customers.length > 0;
      ...
      {
        accessorKey: "customerGroup.id",
        header: () => (
          <div className="text-center">
            <Tooltip>
              <TooltipTrigger>CG</TooltipTrigger>
              <TooltipContent>
                <p>Customer Group</p>
              </TooltipContent>
            </Tooltip>
        ...
        cell: ({ row }) => (
          <div className="font-medium">
            {row?.original?.customerGroup?.groupName ?? "N/A"}
          </div>
        ),
      },
      {
    ...
    [page, limit, customers.length, state.selectedRows],
  );
  ...
  /**
   * When customer create a new customer then close the modal and add the new item to the table
   * @param data
   */
  ...
  /**
   * Trigger when customer update
   * @param data Customer
   */
  const onUpdateSuccess = (data: Customer) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !customers?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
      <Card className="rounded-2xl shadow-sm">
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              ...
              <div>
                ...
                <h3 className="text-gray-500">See and manage your customers</h3>
              </div>
            </div>
            <ButtonGroup>
              <CreateCustomer
          ...
          <div className="grid grid-cols-2 gap-4 mb-3">
            ...
            />
            <CustomerGroupFilter
              value={customerGroupId ?? null}
              onChange={(id) =>
                dispatch({ type: "SET_CUSTOMER_GROUP_ID", payload: id })
              }
            />
          </div>
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these customers?"
      />
      <DeleteModal
        open={deleteOpen}
