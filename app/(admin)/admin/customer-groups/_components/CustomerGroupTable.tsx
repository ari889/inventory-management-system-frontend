...
import UpdateCustomerGroupModal from "./UpdateCustomerGroupModal";
...
import {
  ...
  getCustomerGroups,
} from "@/actions/CustomerGroupAction";
import { customerGroupReducer } from "@/reducers/customerGroupReducer";
import { initialCustomerGroupState } from "@/reducerStates/customerGroupState";
...
export default function CustomerGroupTable() {
   ...
   */
  const fetchCustomerGroupsDebounced = useCallback(
    debounce(async (page: number, limit: number) => {
      ...
      try {
        ...
        dispatch({ type: "SET_CUSTOMER_GROUPS", payload: data.data.items });
   ...
   */
  useEffect(() => {
    fetchCustomerGroupsDebounced(page, limit);

    return () => {
      fetchCustomerGroupsDebounced.cancel();
    };
  }, [page, limit, fetchCustomerGroupsDebounced]);

  /**
   * delete customer group by id
   */
  const deleteCustomerGroup = useCallback(async () => {
    dispatch({ type: "SET_DELETE_LOADING", payload: true });
    try {
      const data = await deleteCustomerGroupById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
      toast.success(data.message, {
        position: "top-right",
      });
      if (customerGroups.length === 1 && page > 1)
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteCustomerGroups(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchCustomerGroupsDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<CustomerGroup>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === customerGroups.length &&
    ...
    [page, limit, customerGroups.length, state.selectedRows],
  );
  ...
  /**
   * When customer group create a new customer group then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: CustomerGroup) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  const onUpdateSuccess = (data: CustomerGroup) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !customerGroups?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
            ...
            <div className="flex flex-row justify-start items-center">
              ...
              <div>
                ...
                <h3 className="text-gray-500">
                  See and manage your customer groups
                </h3>
              </div>
            </div>
            <ButtonGroup>
      ...
      <DeleteModal
        ...
        action={bulkDelete}
        title="Delete all customer groups!"
        description="Are you sure you want to delete all these customer groups?"
      />
      <DeleteModal
        open={deleteOpen}
