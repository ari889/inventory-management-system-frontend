...
const ShowPurchasePaymentModal = ({
  ...
  const fetchWarehouse = useEffectEvent(async () => {
    setLoading(true);
    try {
      const data = await getPurchasePaymentById(purchase?.id);
  ...
  const deletePayment = async () => {
    ...
    try {
      const data = await deletePurchasePaymentById(selectedId!);
