...
import { initialWarehouseState } from "@/reducerStates/warehouseState";
...
export default function WarehouseTable() {
   ...
   */
  const fetchWarehousesDebounced = useCallback(
    debounce(async (page: number, limit: number) => {
      ...
      try {
        ...
        dispatch({ type: "SET_WAREHOUSES", payload: data.data.items });
   ...
   */
  useEffect(() => {
    fetchWarehousesDebounced(page, limit);

    return () => {
      fetchWarehousesDebounced.cancel();
    };
  }, [page, limit, fetchWarehousesDebounced]);
  ...
  const deleteWarehouse = useCallback(async () => {
    ...
    try {
      const data = await deleteWarehouseById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchWarehousesDebounced, warehouses.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteWarehouses(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchWarehousesDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Warehouse>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === warehouses.length &&
            warehouses.length > 0;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">{row.getValue("address") ?? "N/A"}</div>
        ),
      },
      {
        accessorKey: "status",
    ...
    [page, limit, warehouses.length, state.selectedRows],
  );
  ...
  /**
   * When warehouse create a new warehouse then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: Warehouse) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  const onUpdateSuccess = (data: Warehouse) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !warehouses?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
        ...
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              <WarehouseIcon className="mr-2 border rounded border-gray-300 p-2 w-12 h-12" />
              <div>
                <h2 className="text-xl font-semibold">Warehouses</h2>
                <h3 className="text-gray-500">
                  See and manage your warehouses
                </h3>
              </div>
            </div>
            <ButtonGroup>
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these warehouses?"
      />
      <DeleteModal
        open={deleteOpen}
