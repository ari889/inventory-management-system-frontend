...
import { initialAccountState } from "@/reducerStates/accountState";
import {
  bulkDeleteAccounts,
  deleteAccountById,
  getAccounts,
...
export default function AccountTable() {
  ...
  const fetchAccountsDebounced = useCallback(
    debounce(async (page: number, limit: number) => {
      ...
      try {
        ...
        dispatch({ type: "SET_ACCOUNTS", payload: data.data.items });
   ...
   */
  useEffect(() => {
    fetchAccountsDebounced(page, limit);

    return () => {
      fetchAccountsDebounced.cancel();
    };
  }, [page, limit, fetchAccountsDebounced]);
  ...
  const deleteAccount = useCallback(async () => {
    ...
    try {
      const data = await deleteAccountById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchAccountsDebounced, accounts.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteAccounts(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchAccountsDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Account>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === accounts.length && accounts.length > 0;
          ...
          return (
            <Checkbox
              ...
              onCheckedChange={() => {
                const allSelected =
                  state.selectedRows.size === accounts.length &&
                  accounts.length > 0;
                if (allSelected) {
                  dispatch({ type: "DESELECT_ALL_ROWS" });
                } else {
      ...
      },
      {
        ...
        header: ({ column }) => (
          <Button
            ...
            className="px-0"
          >
            Account Number <ArrowUpDown className="ml-2 h-4 w-4" />
          </Button>
        ),
        cell: ({ row }) => (
          <div className="font-medium">
            {row.getValue("accountNo") ?? "N/A"}
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">
            {row.getValue("initialBalance") ?? "N/A"}
          </div>
        ),
      },
      {
    ...
    [page, limit, accounts.length, state.selectedRows],
  );
  ...
  /**
   * When accountr create a new accountr then close the modal and add the new item to the table
   * @param data
   */
  ...
  /**
   * Trigger when account update
   * @param data Account
   */
  const onUpdateSuccess = (data: Account) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !accounts?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
          ...
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              ...
              <div>
                ...
                <h3 className="text-gray-500">See and manage your account</h3>
              </div>
            </div>
            <ButtonGroup>
              <CreateAccount
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these accounts?"
      />
      <DeleteModal
        open={deleteOpen}
