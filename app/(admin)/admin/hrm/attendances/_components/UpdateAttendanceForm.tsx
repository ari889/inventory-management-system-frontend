...
const UpdateAttendanceForm = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      checkIn: format(data?.checkIn, "HH:mm:ss"),
      checkOut: data?.checkOut ? format(data?.checkOut, "HH:mm:ss") : null,
      status: data?.status || false,
    },
    resolver: zodResolver(attendanceSchema),
  });

  const onSubmit = (payload: AttendanceSchemaType) =>
    startTransition(async () => {
      try {
        const response = await updateAttendance(data.id, payload);
