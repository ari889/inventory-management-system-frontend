...
import { initialAttendanceState } from "@/reducerStates/attendanceState";
import {
...
export default function AttendanceTable() {
   ...
   */
  const fetchAttendancesDebounced = useCallback(
   ...
   */
  useEffect(() => {
    fetchAttendancesDebounced(page, limit);

    return () => {
      fetchAttendancesDebounced.cancel();
    };
  }, [page, limit, fetchAttendancesDebounced]);
  ...
  const deleteAttendance = useCallback(async () => {
    ...
    try {
      const data = await deleteAttendanceById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchAttendancesDebounced, attendances.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteAttendances(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchAttendancesDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Attendance>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === attendances.length &&
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">
            {format(row.getValue("date"), "MMMM do, yyyy") ?? "N/A"}
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">
            {format(row.getValue("checkIn"), "hh:mm a") ?? "N/A"}
          </div>
        ),
      },
      {
        ...
        header: ({ column }) => (
          <Button
            ...
            className="px-0"
          >
            Check Out <ArrowUpDown className="ml-2 h-4 w-4" />
          </Button>
        ),
        cell: ({ row }) => (
          <div className="font-medium">
            {row.getValue("checkOut")
              ? format(row.getValue("checkOut"), "hh:mm a")
              : "N/A"}
          </div>
        ),
      },
    ...
    [page, limit, attendances.length, state.selectedRows],
  );
  ...
  /**
   * When attendance create a new attendance then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: Attendance) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  /**
   * Trigger when attendance update
   * @param data Attendance
   */
  const onUpdateSuccess = (data: Attendance) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !attendances?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
        ...
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              <BriefcaseBusiness className="mr-2 border rounded border-gray-300 p-2 w-12 h-12" />
              <div>
                <h2 className="text-xl font-semibold">Attendances</h2>
                <h3 className="text-gray-500">
                  See and manage your attendances
                </h3>
              </div>
            </div>
            <ButtonGroup>
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these attendances?"
      />
      <DeleteModal
        open={deleteOpen}
