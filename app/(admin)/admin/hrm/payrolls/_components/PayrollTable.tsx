...
import { initialPayrollState } from "@/reducerStates/payrollState";
import {
  bulkDeletePayrolls,
  deletePayrollById,
  getPayrolls,
...
export default function PayrollTable() {
   ...
   */
  const fetchPayrollsDebounced = useCallback(
   ...
   */
  useEffect(() => {
    fetchPayrollsDebounced(page, limit);

    return () => {
      fetchPayrollsDebounced.cancel();
    };
  }, [page, limit, fetchPayrollsDebounced]);
  ...
  const deletePayroll = useCallback(async () => {
    ...
    try {
      const data = await deletePayrollById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
      toast.success(data.message, {
        position: "top-right",
      });
      if (payrolls.length === 1 && page > 1)
  ...
  }, [selectedId, page, limit, fetchPayrollsDebounced, payrolls.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeletePayrolls(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchPayrollsDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Payroll>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === payrolls.length && payrolls.length > 0;
          ...
          return (
            <Checkbox
              ...
              onCheckedChange={() => {
                const allSelected =
                  state.selectedRows.size === payrolls.length &&
                  payrolls.length > 0;
                if (allSelected) {
                  dispatch({ type: "DESELECT_ALL_ROWS" });
                } else {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">{row.getValue("amount") ?? "N/A"}</div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">
            {row.getValue("paymentMethods") ?? "N/A"}
          </div>
        ),
      },
      {
    ...
    [page, limit, payrolls.length, state.selectedRows],
  );
  ...
  /**
   * When payroll create a new payroll then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: Payroll) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  const onUpdateSuccess = (data: Payroll) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !payrolls?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
        ...
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              <BanknoteArrowDown className="mr-2 border rounded border-gray-300 p-2 w-12 h-12" />
              <div>
                <h2 className="text-xl font-semibold">Payrolls</h2>
                <h3 className="text-gray-500">See and manage your payroll</h3>
              </div>
            </div>
            <ButtonGroup>
              <CreatePayroll
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these payrolls?"
      />
      <DeleteModal
        open={deleteOpen}
