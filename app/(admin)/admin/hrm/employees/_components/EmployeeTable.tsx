...
import { initialEmployeeState } from "@/reducerStates/employeeState";
import {
  bulkDeleteEmployees,
  deleteEymployeeById,
  getEmployees,
...
import DepartmentFilter from "@/components/common/filter/DepartmentFilter";
...
export default function EmployeeTable() {
  ...
  const fetchEmployeesDebounced = useCallback(
    debounce(async (page: number, limit: number) => {
      ...
      try {
        ...
        dispatch({ type: "SET_EMPLOYEES", payload: data.data.items });
    ...
    [page, limit, sorting, search, status, createdBy, departmentId],
  );
   ...
   */
  useEffect(() => {
    fetchEmployeesDebounced(page, limit);

    return () => {
      fetchEmployeesDebounced.cancel();
    };
  }, [page, limit, fetchEmployeesDebounced]);
  ...
  const deleteEmployee = useCallback(async () => {
    ...
    try {
      const data = await deleteEymployeeById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchEmployeesDebounced, employees.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteEmployees(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchEmployeesDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Employee>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === employees.length &&
            employees.length > 0;
    ...
    [page, limit, employees.length, state.selectedRows],
  );
  ...
  /**
   * When employee create a new employee then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: Employee) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  const onUpdateSuccess = (data: Employee) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !employees?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
        ...
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              ...
              <div>
                ...
                <h3 className="text-gray-500">See and manage your employees</h3>
              </div>
            </div>
            <ButtonGroup>
              <CreateEmployee
          ...
          <div className="grid grid-cols-2 gap-4 mb-3">
            ...
            <DepartmentFilter
              ...
              onChange={(id) =>
                dispatch({ type: "SET_DEPARTMENT_ID", payload: id })
              }
            />
          </div>
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these employees?"
      />
      <DeleteModal
        open={deleteOpen}
