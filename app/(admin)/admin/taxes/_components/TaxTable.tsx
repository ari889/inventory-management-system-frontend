...
export default function TaxTable() {
   ...
   */
  const fetchTaxesDebounced = useCallback(
   ...
   */
  useEffect(() => {
    fetchTaxesDebounced(page, limit);

    return () => {
      fetchTaxesDebounced.cancel();
    };
  }, [page, limit, fetchTaxesDebounced]);
  ...
  const deleteTax = useCallback(async () => {
    ...
    try {
      const data = await deleteTaxById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
      toast.success(data.message, {
        position: "top-right",
      });
      if (taxes.length === 1 && page > 1)
  ...
  }, [selectedId, page, limit, fetchTaxesDebounced, taxes.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteTaxes(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchTaxesDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Tax>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === taxes.length && taxes.length > 0;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">
            {row.getValue("rate") ? `${row.getValue("rate")}%` : "N/A"}
          </div>
        ),
      },
      {
    ...
    [page, limit, taxes.length, state.selectedRows],
  );
  ...
  /**
   * When tax create a new tax then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: Tax) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  const onUpdateSuccess = (data: Tax) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !taxes?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
        ...
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              <Percent className="mr-2 border rounded border-gray-300 p-2 w-12 h-12" />
              <div>
                <h2 className="text-xl font-semibold">Taxes</h2>
                <h3 className="text-gray-500">See and manage your taxes</h3>
              </div>
            </div>
            <ButtonGroup>
              <CreateTax
      ...
      <DeleteModal
        ...
        action={bulkDelete}
        title="Delete all taxes!"
        description="Are you sure you want to delete all these taxes?"
      />
      <DeleteModal
        open={deleteOpen}
