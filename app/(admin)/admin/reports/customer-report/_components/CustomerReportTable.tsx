...
import { initialCustomerReportState } from "@/reducerStates/customerReportState";
import { getCustomerReport } from "@/actions/ReportAction";
...
export default function CustomerReportTable() {
   ...
   */
  const fetchCustomerReportsDebounced = useCallback(
    ...
    [page, limit, sorting, dateRange, saleNo, customerId],
  );
   ...
   */
  useEffect(() => {
    fetchCustomerReportsDebounced(page, limit);

    return () => {
      fetchCustomerReportsDebounced.cancel();
    };
  }, [page, limit, fetchCustomerReportsDebounced]);
  ...
  const totalGrandTotal = sales?.reduce((acc, curr) => {
    return acc + Number(curr?.grandTotal);
  }, 0);

  const totalPaidAmount = sales?.reduce((acc, curr) => {
    return acc + Number(curr?.paidAmount);
  }, 0);

  const totalDueAcount = sales?.reduce((acc, curr) => {
            ...
            <div className="flex flex-row justify-start items-center">
              ...
              <div>
                ...
                <h3 className="text-gray-500">
                  See and manage your customer report
                </h3>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4 mb-3">
            ...
            <Field className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  dispatch({ type: "SET_SALE_NO", payload: e.target.value })
                }
              />
            </Field>
            <CustomerFilter
