// components/reports/DailyPurchasesReport.tsx
"use client";
...
import DailyPurchasesReportSkeleton from "./DailyPurchaseReportSkeleton";
...
export default function DailyPurchaseReport() {
  ...
  const fetchDailyPurchases = useCallback(
    async (from?: Date, to?: Date) => {
      setError("");
      setLoading(true);
      try {
        const data = await getDailyPurchase(warehouseId, from, to);
        if (!data?.success)
  ...
  const debouncedFetch = useMemo(
    () =>
      debounce((from?: Date, to?: Date) => fetchDailyPurchases(from, to), 300),
    [fetchDailyPurchases],
  );
   ...
   */
  let content = null;
  if (loading) content = <DailyPurchasesReportSkeleton />;
