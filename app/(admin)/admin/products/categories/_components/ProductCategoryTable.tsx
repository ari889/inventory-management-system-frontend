...
import { initialProductCategoryState } from "@/reducerStates/productCategoryState";
import {
...
export default function ProductCategoryTable() {
   ...
   */
  useEffect(() => {
    fetchProductCategoriesDebounced(page, limit);

    return () => {
      fetchProductCategoriesDebounced.cancel();
    };
  }, [page, limit, fetchProductCategoriesDebounced]);
  ...
  const deleteProductCategory = useCallback(async () => {
    ...
    try {
      const data = await deleteProductCategoryById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
      toast.success(data.message, {
        position: "top-right",
      });
      if (productCategoris.length === 1 && page > 1)
    ...
    page,
    limit,
    fetchProductCategoriesDebounced,
    productCategoris.length,
  ]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteProductCategories(
      ...
      fetchProductCategoriesDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<ProductCategory>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === productCategoris.length &&
            productCategoris.length > 0;
    ...
    [page, limit, productCategoris.length, state.selectedRows],
  );
  ...
  /**
   * When product category create a new product category then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: ProductCategory) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  /**
   * Trigger when product category update
   * @param data ProductCategory
   */
  const onUpdateSuccess = (data: ProductCategory) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !productCategoris?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
            ...
            <div className="flex flex-row justify-start items-center">
              ...
              <div>
                ...
                <h3 className="text-gray-500">
                  See and manage your product categories
                </h3>
              </div>
            </div>
            <ButtonGroup>
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these product categories?"
      />
      <DeleteModal
        open={deleteOpen}
