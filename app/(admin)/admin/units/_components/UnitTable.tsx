...
import { initialUnitState } from "@/reducerStates/unitState";
import {
  bulkDeleteUnits,
  deleteUnitById,
  getUnits,
...
export default function UnitTable() {
   ...
   */
  const fetchUnitsDebounced = useCallback(
   ...
   */
  useEffect(() => {
    fetchUnitsDebounced(page, limit);

    return () => {
      fetchUnitsDebounced.cancel();
    };
  }, [page, limit, fetchUnitsDebounced]);
  ...
  const deleteUnit = useCallback(async () => {
    ...
    try {
      const data = await deleteUnitById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchUnitsDebounced, units.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteUnits(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchUnitsDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Unit>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === units.length && units.length > 0;
      ...
      },
      {
        ...
        header: ({ column }) => (
          <Button
            ...
            className="px-0"
          >
            Unit Name <ArrowUpDown className="ml-2 h-4 w-4" />
          </Button>
        ),
        cell: ({ row }) => (
          <div className="font-medium">{row.getValue("unitName") ?? "N/A"}</div>
        ),
      },
      {
        ...
        header: ({ column }) => (
          <Button
            ...
            className="px-0"
          >
            Unit Code <ArrowUpDown className="ml-2 h-4 w-4" />
          </Button>
        ),
        cell: ({ row }) => (
          <div className="font-medium">{row.getValue("unitCode")}</div>
        ),
      },
      {
        accessorKey: "baseUnit.id",
        header: () => <div className="text-center">Base Unit</div>,
        cell: ({ row }) => (
          <div className="font-medium">
            {row?.original?.baseUnit?.unitName ?? "N/A"}
          </div>
        ),
      },
      {
        ...
        header: ({ column }) => (
          <Button
            ...
            className="px-0"
          >
            Operator <ArrowUpDown className="ml-2 h-4 w-4" />
          </Button>
        ),
        cell: ({ row }) => (
          <div className="font-medium">{row.getValue("operator") ?? "N/A"}</div>
        ),
      },
      {
        ...
        header: ({ column }) => (
          <Button
            ...
            className="px-0"
          >
            Operation Value <ArrowUpDown className="ml-2 h-4 w-4" />
          </Button>
        ),
        cell: ({ row }) => (
          <div className="font-medium">
            {row.getValue("operationValue") ?? "N/A"}
          </div>
        ),
      },
      {
    ...
    [page, limit, units.length, state.selectedRows],
  );
  ...
  /**
   * When unit create a new unit then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: Unit) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  const onUpdateSuccess = (data: Unit) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !units?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
        ...
        <CardContent>
          <div className="flex flex-row justify-between items-center my-3">
            <div className="flex flex-row justify-start items-center">
              <Weight className="mr-2 border rounded border-gray-300 p-2 w-12 h-12" />
              <div>
                <h2 className="text-xl font-semibold">Units</h2>
                <h3 className="text-gray-500">See and manage your units</h3>
              </div>
            </div>
            <ButtonGroup>
              <CreateUnit
      ...
      <DeleteModal
        ...
        description="Are you sure you want to delete all these units?"
      />
      <DeleteModal
        open={deleteOpen}
