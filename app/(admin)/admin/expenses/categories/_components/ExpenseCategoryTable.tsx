...
import { initialExpenseCategoryState } from "@/reducerStates/expenseCategoryState";
import {
...
export default function ExpenseCategoryTable() {
   ...
   */
  const fetchExpenseCategoriesDebounced = useCallback(
   ...
   */
  useEffect(() => {
    fetchExpenseCategoriesDebounced(page, limit);

    return () => {
      fetchExpenseCategoriesDebounced.cancel();
    };
  }, [page, limit, fetchExpenseCategoriesDebounced]);

  /**
   * delete expense category by id
   */
  const deleteExpenseCategory = useCallback(async () => {
    dispatch({ type: "SET_DELETE_LOADING", payload: true });
    try {
      const data = await deleteExpenseCategoryById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
      toast.success(data.message, {
        position: "top-right",
      });
      if (expenseCategoris.length === 1 && page > 1)
    ...
    page,
    limit,
    fetchExpenseCategoriesDebounced,
    expenseCategoris.length,
  ]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteExpenseCategories(
      ...
      fetchExpenseCategoriesDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<ExpenseCategory>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === expenseCategoris.length &&
            expenseCategoris.length > 0;
    ...
    [page, limit, expenseCategoris.length, state.selectedRows],
  );
  ...
  /**
   * When expense category create a new expense category then close the modal and add the new item to the table
   * @param data
   */
  const onSuccess = (data: ExpenseCategory) => {
    if (page === 0) {
      dispatch({
        type: "REFRESH",
        payload: data,
  ...
  /**
   * Trigger when expense category update
   * @param data ExpenseCategory
   */
  const onUpdateSuccess = (data: ExpenseCategory) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !expenseCategoris?.length)
    content = (
      <TableAlert
  ...
  return (
    <div className="p-6">
      ...
      <DeleteModal
        ...
        action={bulkDelete}
        title="Delete all expense categories!"
        description="Are you sure you want to delete all these expense categories?"
      />
      <DeleteModal
        open={deleteOpen}
