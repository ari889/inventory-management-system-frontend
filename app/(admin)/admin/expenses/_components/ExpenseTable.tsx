...
import { initialExpenseState } from "@/reducerStates/expenseState";
import {
  bulkDeleteExpense,
  deleteExpenseById,
  getExpense,
...
export default function ExpenseTable() {
   ...
   */
  const fetchExpenseDebounced = useCallback(
   ...
   */
  useEffect(() => {
    fetchExpenseDebounced(page, limit);

    return () => {
      fetchExpenseDebounced.cancel();
    };
  }, [page, limit, fetchExpenseDebounced]);
  ...
  const deleteExpense = useCallback(async () => {
    ...
    try {
      const data = await deleteExpenseById(selectedId!);
      if (!data?.success && !data?.errors) throw new Error(data.message);
  ...
  }, [selectedId, page, limit, fetchExpenseDebounced, expenses.length]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const response = await bulkDeleteExpense(Array.from(selectedRows));
      if (!response.success) throw new Error(response.message);
      dispatch({ type: "DESELECT_ALL_ROWS" });
      dispatch({ type: "TOGGLE_BULK_DELETE_MODAL" });
      dispatch({ type: "SET_PAGE", payload: 0 });
      fetchExpenseDebounced(0, limit);
  ...
  const columns = useMemo<ColumnDef<Expense>[]>(
    () => [
      {
        ...
        header: () => {
          const allSelected =
            state.selectedRows.size === expenses.length && expenses.length > 0;
          ...
          return (
            <Checkbox
              ...
              onCheckedChange={() => {
                const allSelected =
                  state.selectedRows.size === expenses.length &&
                  expenses.length > 0;
                if (allSelected) {
                  dispatch({ type: "DESELECT_ALL_ROWS" });
                } else {
      ...
      {
        accessorKey: "expenseCategory.id",
        header: () => <div className="text-center">Expense Category</div>,
        cell: ({ row }) => (
          <div className="font-medium">
            {row?.original?.expenseCategory?.name ?? "N/A"}
          </div>
        ),
      },
      {
    ...
    [page, limit, expenses.length, state.selectedRows],
  );
  ...
  /**
   * When expense create a new expense then close the modal and add the new item to the table
   * @param data
   */
  ...
  /**
   * Trigger when expense update
   * @param data Expense
   */
  const onUpdateSuccess = (data: Expense) => {
    dispatch({
      type: "UPDATE_SUCCESS",
      payload: data,
    });
  ...
  if (!isLoading && !isError && !expenses?.length)
    content = (
      <TableAlert
