...
/**
 * A reusable autocomplete component for selecting a brand. Fetches options from the server with support for searching and infinite scrolling.
 */
...
/**
 * State management for the BrandAutocomplete component using useReducer. Handles loading states, error handling, and pagination for fetching brands.
 */
type State = {
  brands: Brand[];
  loading: boolean;
...
/**
 * Actions for the reducer to manage the state of BrandAutocomplete, including fetching brands, handling search input, and managing the open state of the popover.
 */
type Action =
...
/**
 * Reducer function to manage the state of the BrandAutocomplete component. Handles different action types to update the state accordingly, such as starting a fetch, successfully fetching data, handling errors, and updating search input.
...
export default function BrandAutocomplete<T extends FieldValues>({
  ...
  /**
   * Fetch brands from the server with support for pagination and searching. Dispatches actions to update the state based on the fetch status (loading, success, error) and whether it's an initial load, a search, or loading more items.
   * @param nextPage
   * @param nextSearch
   * @param replace
   */
  ...
  /**
   * Initial fetch of brands when the component mounts. This ensures that the autocomplete has options to display when the user interacts with it for the first time. The empty dependency array ensures this effect runs only once on mount.
   */
  useEffect(() => {
    fetchBrands(0, "", true);
  }, []);
  ...
  /**
   * Set the default selected brand when the component mounts or when the defaultBrand prop changes. This ensures that if an initial brand is provided from the parent component, it will be displayed as the selected option in the autocomplete.
   */
  useEffect(() => {
