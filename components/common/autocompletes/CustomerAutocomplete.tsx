...
/**
 * A reusable autocomplete component for selecting a customer. Fetches options from the server with support for searching and infinite scrolling.
 */
...
/**
 * State management for the CustomerAutocomplete component using useReducer. Handles loading states, error handling, and pagination for fetching customers.
 */
type State = {
  customers: Customer[];
  loading: boolean;
...
/**
 * Actions for the reducer to manage the state of CustomerAutocomplete, including fetching customers, handling search input, and managing the open state of the popover.
 */
type Action =
...
/**
 * Reducer function to manage the state of the CustomerAutocomplete component. Handles different action types to update the state accordingly, such as starting a fetch, successfully fetching data, handling errors, and updating search input.
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "FETCH_SUCCESS": {
      ...
      return {
        ...state,
        customers: replace ? items : [...state.customers, ...items],
...
export default function CustomerAutocomplete<T extends FieldValues>({
  control,
  name,
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<Pick<
    Customer,
    "id" | "name"
  ...
  /**
   * Fetch customers from the server with support for pagination and searching. Dispatches actions to update the state based on the fetch status (loading, success, error) and whether it's an initial load, a search, or loading more items.
   * @param nextPage
   * @param nextSearch
   * @param replace
   */
    ...
    <Controller
      ...
      render={({ field, fieldState }) => {
        const selectedLabel =
          ...
          state.customers.find((r) => r.id === field.value)?.name;
                          ...
                          {state.customers.map((customer) => (
                            <CommandItem
                              ...
                              onSelect={(val) => {
                                ...
                                const picked =
                                  state.customers.find((u) => u.id === num) ??
                                  null;
                                setSelectedCustomer(
