...
export default function UserFilter({
  ...
  const handleScroll = useCallback(() => {
    ...
    if (nearBottom) {
      ...
      fetchUsers(nextPage, searchRef.current, false);
    }
  }, [state.loadingMore, state.hasMore]);

  const selectedLabel =
    selectedUser?.name ?? state.users.find((r) => r.id === value)?.name;
                    ...
                    {state.users.map((user) => (
                      <CommandItem
                        ...
                        onSelect={(val) => {
                          ...
                          const picked =
                            state.users.find((u) => u.id === num) ?? null;
                          const isDeselecting = num === value;
                          setSelectedUser(isDeselecting ? null : picked);
