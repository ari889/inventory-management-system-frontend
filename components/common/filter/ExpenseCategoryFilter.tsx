...
type CategoryOption = Pick<ExpenseCategory, "id" | "name">;

type Props = {
  value: number | null;
  onChange: (value: number | null) => void;
  label?: string;
  defaultCategory?: CategoryOption | null;
};

type State = {
  expenseCtegories: ExpenseCategory[];
  loading: boolean;
  searching: boolean;
  loadingMore: boolean;
  hasMore: boolean;
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "FETCH_SUCCESS": {
      ...
      return {
        ...state,
        expenseCtegories: replace
          ? items
          : [...state.expenseCtegories, ...items],
...
export default function ExpenseCategoryFilter({
  ...
  const handleScroll = useCallback(() => {
    ...
    if (nearBottom) {
      ...
      fetchCategorys(nextPage, searchRef.current, false);
    }
  }, [state.loadingMore, state.hasMore]);

  const selectedLabel =
    selectedCategory?.name ??
    state.expenseCtegories.find((r) => r.id === value)?.name;
              ...
              <CommandList ref={listRef} onScroll={handleScroll}>
                {state.searching ? (
                  ...
                  <>
                    ...
                    {state.expenseCtegories.map((category) => (
                      <CommandItem
                        key={category.id}
                        ...
                        onSelect={(val) => {
                          ...
                          const picked =
                            state.expenseCtegories.find((u) => u.id === num) ??
                            null;
                          const isDeselecting = num === value;
                          setSelectedCategory(isDeselecting ? null : picked);
