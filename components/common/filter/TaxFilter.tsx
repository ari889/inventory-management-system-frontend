...
type TaxOption = Pick<Tax, "id" | "name">;

type Props = {
  value: number | null;
  onChange: (value: number | null) => void;
  label?: string;
  defaultTax?: TaxOption | null;
};
...
export default function TaxFilter({
  ...
  const [selectedTax, setSelectedTax] = useState<TaxOption | null>(defaultTax);
  ...
  const selectedLabel =
    selectedTax?.name ?? state.taxs.find((r) => r.id === value)?.name;
                    ...
                    {state.taxs.map((tax) => (
                      <CommandItem
                        ...
                        onSelect={(val) => {
                          ...
                          setSelectedTax(isDeselecting ? null : picked);
